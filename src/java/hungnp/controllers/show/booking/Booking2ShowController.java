 ...
 * @author DELL
 */
public class Booking2ShowController extends HttpServlet {
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			List<TouristDTO> touristListReturn = new ArrayList();
			if (touristList != null){
				for (TouristDTO dto : touristList)
					if (dto.getTourCode().equals(code)){
						touristListReturn.add(dto);
					}
			}
			...
			request.setAttribute("touristList", touristListReturn);
			request.setAttribute("tourDto", tourDto);
			request.setAttribute("tourArticleDto", tourArticleDto);
			
		} catch (Exception e) {
			log("Error at Booking2ShowController: " + e.getMessage());
		} finally{
			request.getRequestDispatcher("/booking/booking-2.jsp").forward(request, response);
		}
	}
